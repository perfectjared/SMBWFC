...
static class STMCustomInspectorTools {
	...
	*/
	public static void FinishItem(Object myObject){
	...
	public static void DrawMaterialEditor(Material mat){
		...
		if(mat.GetTag("STMUberShader", true, "Null") == "Yes" && mat.HasProperty("_ShadowAngle3"))
		{
			//toggle use vector 3
			bool useVector3 = mat.IsKeywordEnabled("USE_VECTOR3");
			EditorGUI.BeginChangeCheck();
			useVector3 = EditorGUILayout.Toggle("Use Vector3", useVector3);//show the toggle
			if(EditorGUI.EndChangeCheck())
			{
				mat.SetFloat("_UseVector3", useVector3 ? 1 : 0); //call this too so newer unity versions don't break
				if(useVector3)
				{
					mat.EnableKeyword("USE_VECTOR3");
				}
				else
				{
					mat.DisableKeyword("USE_VECTOR3");
				}
				//EditorUtility.SetDirty(mat);
			}
			if(useVector3)
